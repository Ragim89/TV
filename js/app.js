...
(function () {
  ...
  /* Короткий день недели: полный не помещается в строку рядом с иконками. */
  ...
  function renderInstall() {
    ...
    if (deferredPrompt) {
      txt($('#install-text'), 'Поставь на экран «Домой»: откроется на весь экран и без сети.');
      $('#install-go').hidden = false;
      txt($('#install-no'), 'Не сейчас');
    } else if (ios) {
      txt($('#install-text'), 'Нажми «Поделиться» внизу Safari и выбери «На экран „Домой“».');
      $('#install-go').hidden = true;
      txt($('#install-no'), 'Понятно');
    } else {
      txt($('#install-text'), 'Открой меню браузера и выбери «Установить приложение».');
      $('#install-go').hidden = true;
      txt($('#install-no'), 'Понятно');
    }
    box.hidden = false;
